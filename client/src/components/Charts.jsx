...
export function TrendChart({ data = [], loading }) {
  ...
  if (!data || data.length === 0) return (
    <div className="h-[260px] flex items-center justify-center bg-slate-50 border border-dashed border-slate-200 rounded-lg">
      <span className="text-slate-400 text-sm font-medium">{t('No complaint data available for this period.')}</span>
    </div>
  );
  ...
  return (
    <ResponsiveContainer width="100%" height={260}>
      <LineChart data={formatted} margin={{ top: 5, right: 20, left: 0, bottom: 5 }}>
        ...
        <Line type="monotone" name={t('Submitted')} dataKey="Submitted" stroke="#1A3A6B" strokeWidth={2} dot={false} />
        <Line type="monotone" name={t('Resolved')} dataKey="Resolved" stroke="#16A34A" strokeWidth={2} dot={false} strokeDasharray="4 4" />
      </LineChart>
    </ResponsiveContainer>
  );
}
  ...
  return (
    <ResponsiveContainer width="100%" height={260}>
      <BarChart data={data} margin={{ top: 5, right: 20, left: 0, bottom: 5 }}>
        <CartesianGrid strokeDasharray="3 3" stroke="#E2E8F0" />
        <XAxis dataKey="_id" tick={{ fontSize: 10, fill: '#64748B' }} tickFormatter={(val) => t(val)} />
        <YAxis tick={{ fontSize: 11, fill: '#64748B' }} />
        <Tooltip contentStyle={{ fontSize: 12, borderRadius: 8 }} formatter={(value, name, props) => [value, t(props.payload._id)]} />
        <Bar dataKey="count" name={t('Complaints')} radius={[4, 4, 0, 0]}>
          {data.map((_, index) => (
    ...
    <ResponsiveContainer width="100%" height={260}>
      <PieChart>
        <Pie
          ...
          nameKey="_id"
          label={({ _id, percent }) => {
            const translated = translateDepartment(_id || 'Unknown');
            const words = translated.split(' ');
            const shortLabel = words[0] === 'ज़ोन' || words[0] === 'Zone' ? `${words[0]} ${words[1]}` : words[0];
        ...
        <Tooltip formatter={(value, name, props) => [value, t(props.payload._id) || t('Unknown')]} />
      </PieChart>
    </ResponsiveContainer>
  );
}
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-slate-200 bg-slate-50">
            ...
            <th className="text-right px-4 py-3 text-slate-600 font-semibold">{t('Total')}</th>
            <th className="text-right px-4 py-3 text-slate-600 font-semibold">{t('Resolved')}</th>
            <th className="text-right px-4 py-3 text-slate-600 font-semibold">{t('Pending')}</th>
            <th className="px-4 py-3 text-slate-600 font-semibold">{t('SLA Compliance')}</th>
          </tr>
        </thead>
        <tbody>
          {data.map((dept, i) => {
            ...
            return (
              <tr key={i} className="border-b border-slate-100 hover:bg-slate-50">
                <td className="px-4 py-3 font-medium text-slate-800">{dept._id ? translateDepartment(dept._id) : t('Not assigned')}</td>
